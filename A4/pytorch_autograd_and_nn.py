"""
Implements ResNet for CIFAR-10 with PyTorch Autograd and NN in PyTorch.
...
def three_layer_convnet(x, params, hyperparams):
    ...
    x = F.relu(F.conv2d(input=x, weight=conv_w1, bias=conv_b1, padding=pad_size_1))
    x = F.relu(F.conv2d(input=x, weight=conv_w2, bias=conv_b2, padding=pad_size_2))
    x = torch.flatten(x, start_dim=1)
    scores = F.linear(input=x, weight=fc_w, bias=fc_b)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return scores
...
def initialize_three_layer_conv_part2():
    ...
    ###########################################################################
    # Replace "pass" with your code (do not modify this line)
    model = ThreeLayerConvNet(in_shape=(C, H, W), 
...
def initialize_three_layer_conv_part3():
    ...
    model = nn.Sequential(OrderedDict([
        ...
        ('relu1', nn.ReLU()),
        ('conv2', nn.Conv2d(in_channels=channel_1, out_channels=channel_2, kernel_size=kernel_size_2, padding=pad_size_2)),
        ('relu2', nn.ReLU()),
        ('flatten', nn.Flatten()),
    ...
    def __init__(self, Cin, Cout, downsample=False):
        ...
        if downsample:
            self.net = nn.Sequential(
                nn.BatchNorm2d(Cin),
                nn.ReLU(),
                nn.Conv2d(Cin, Cout, 3, padding=1, stride=2, bias=False),
                nn.BatchNorm2d(Cout),
                nn.ReLU(),
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, Cin, Cout, downsample=False):
        ...
        if downsample:
            self.shortcut = nn.Conv2d(in_channels=Cin, out_channels=Cout, kernel_size=1, 
                                      stride=2, bias=False)
        else:
            if (Cin == Cout):
                self.shortcut = nn.Identity()
            else:
                self.shortcut = nn.Conv2d(in_channels=Cin, 
                                          out_channels=Cout, kernel_size=1, stride=1, bias=False)
        ######################################################################
        #                          END OF YOUR CODE                          #
        ######################################################################
...
class ResNet(nn.Module):
    def __init__(self, stage_args, in_channels=3, block=ResidualBlock,
        ...
        where all convolutional blocks processing spatial features are wrapped
        with self.cnn, global average pooling (and flattening) are wrapped
        with self.avgpool, and the final linear layer is self.fc.
        ...
        # Both input and output of self.cnn are expected to be 4-D tensor;   #
        # e.g., tensor of shape (N, C, H, W) and (N, C', H', W').            #
        # You should multiply block.expansion to the dimension of each stage #
        # to make this compatible with ResidualBottleneck.                   #
        # Implement the global average pooling part (with flattening) in     #
        # self.avgpool. The input and output of self.avgpool is expected to  #
        # be 4-D and 2-D tensor, respectively; e.g., tensor of shape         #
        # (N, C', H', W') and (N, C').                                       #
        # Hint: nn.AdaptiveAvgPool2d, nn.Flatten                             #
        ######################################################################
        # Replace "pass" with your code (do not modify this line)
        layers = []
        layers.append(ResNetStem(Cin=in_channels, Cout=stage_args[0][0]))
        layers.append(ResNetStage(Cin=stage_args[0][0], Cout=stage_args[0][0],
                                  num_blocks=stage_args[0][1], downsample=stage_args[0][2], block=block))
        
        in_C = stage_args[0][0] * block.expansion
        for i in range(1, len(stage_args)):
            out_C = stage_args[i][0]
            layers.append(ResNetStage(Cin=in_C, Cout=out_C,
                                      num_blocks=stage_args[i][1], downsample=stage_args[i][2], block=block))
            in_C = out_C * block.expansion
               
        layers.append(nn.BatchNorm2d(stage_args[-1][0] * block.expansion))
...
class ResidualBottleneck(nn.Module):
    ...
    def __init__(self, Cin, Cout, downsample=False):
        ...
        self.block = nn.Sequential(
            ...
            nn.BatchNorm2d(Cout),
            nn.ReLU(),
            nn.Conv2d(Cout, Cout * self.expansion, kernel_size=1, bias=False)
        )

        if Cin != Cout * self.expansion or downsample:
            self.shortcut = nn.Conv2d(Cin, Cout * self.expansion, kernel_size=1, stride=stride, bias=False)
        else:
            self.shortcut = nn.Identity()
