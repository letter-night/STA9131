"""
Implements VAE and CVAE in PyTorch.
...
class VAE(nn.Module):
    def __init__(self, input_size=784, hidden_dim=256, latent_size=15):
        ...
        ######################################################################
        # Replace "pass" with your code (do not modify this line)
        self.encoder = get_encoder(input_size=input_size, 
                                   hidden_dim=hidden_dim, latent_size=latent_size)
        self.decoder = get_decoder(input_size=input_size, 
                                   hidden_dim=hidden_dim, latent_size=latent_size)
        ######################################################################
        #                          END OF YOUR CODE                          #
        ######################################################################
    ...
    def forward(self, x):
        ...
        # 4. Pass z through the decoder to reconstruct x.                    #
        # Hint: x[:,:S], x[:,S:] slices the second dimension to [0,S), [S,D) #
        ######################################################################
        # Replace "pass" with your code (do not modify this line)
        enc_out = self.encoder(x)
        mu, logvar = enc_out[:, :enc_out.shape[1]//2], enc_out[:, enc_out.shape[1]//2:]
...
def loss_func(x, x_hat, mu, logvar):
    ...
    BCE_loss = F.binary_cross_entropy(input=x_hat, target=x, reduction='sum')
...
class CVAE(nn.Module):
    def __init__(self, input_size=784, hidden_dim=256, latent_size=15,
        ...
        ######################################################################
        # Replace "pass" with your code (do not modify this line)
        self.encoder = get_encoder(input_size=input_size, hidden_dim=hidden_dim, latent_size=latent_size,
                                   num_classes=num_classes)
        self.decoder = get_decoder(input_size=input_size, hidden_dim=hidden_dim, latent_size=latent_size,
                                   num_classes=num_classes)
        ######################################################################
        #                          END OF YOUR CODE                          #
        ######################################################################
    ...
    def forward(self, x, c):
        ...
        ######################################################################
        # Replace "pass" with your code (do not modify this line)
        enc_out = self.encoder(torch.cat((x, c), dim=1))
        mu, logvar = enc_out[:, :enc_out.shape[1]//2], enc_out[:, enc_out.shape[1]//2:]
        z = reparametrize(mu, logvar)
