...
def _extract_tensors(dset, num=None, x_dtype=torch.float32):
    """
    Extract the data and labels from a CIFAR10 dataset object and convert them
    to tensors.
...
def preprocess_cifar10(
    ...
    N_train, N_val, and N_test are the number of examples in the train, val,
    and test sets respectively. The precise values of N_train and N_val are
    determined by the input parameter validation_ratio. D is the dimension of
    the image data;
    ...
    if show_examples:
        ...
        visualize_dataset(X_train, y_train,
                          nrow=nrow, classes=classes, nchw=True)
