...
def pos_enc_simple(seq_len: int, emb_dim: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" with your code (do not modify this line)
    base = torch.arange(seq_len) / seq_len # (seq_len, )
    out = base.unsqueeze(1).repeat(1, emb_dim).unsqueeze(0) # (1, seq_len, emb_dim)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return out
...
def pos_enc_sinusoid(seq_len: int, emb_dim: int) -> Tensor:
    ...
    angle_rads = position * div_term.unsqueeze(0) # (seq_len, emb_dim//2)

    pe = torch.zeros(seq_len, emb_dim)
    pe[:, 0::2] = torch.sin(angle_rads[:, :pe[:, 0::2].shape[1]])
    pe[:, 1::2] = torch.cos(angle_rads[:, :pe[:, 1::2].shape[1]])

    out = pe.unsqueeze(0) # (1, seq_len, emb_dim)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return out
...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(query.shape[0]):
        for j in range(key.shape[0]):
            scores[i][j] = (query[i] @ key[j]) * 1 / torch.sqrt(torch.tensor(emb_dim))
        scores[i] = F.softmax(scores[i], dim=0)
...
def scaled_dot_product_two_loop_batch(
    ...
    # using two for-loops. Specifically, follow the steps below:             #
    # 1. For each query, compute dot product with each key in the sequence,  #
    ...
    ##########################################################################
    # Replace "pass" with your code (do not modify this line)
    scale = 1.0 / torch.sqrt(torch.tensor(emb_dim, dtype=torch.float32))

    out = torch.zeros(batch_size, seq_len, emb_dim)

    for b in range(batch_size):
        attn_scores = torch.zeros(seq_len, seq_len) 

        for i in range(seq_len):
            attn_scores[i] = torch.matmul(key[b], query[b, i]) # (seq_len,)
        
        attn_scores = attn_scores * scale

        attn_weights = torch.softmax(attn_scores, dim=1) # (seq_len, seq_len)

        attn_weights = attn_weights.unsqueeze(0) # (1, seq_len, seq_len)
        V = value[b].unsqueeze(0) # (1, seq_len, emb_dim)
        out[b] = torch.bmm(attn_weights, V).squeeze(0) # (seq_len, emb_dim)
...
def scaled_dot_product_no_loop_batch(
    ...
    ##########################################################################
    # TODO: Implement scaled dot-product attention for a batch of inputs     #
    # **without** for-loops. Specifically, follow the steps below:           #
    ...
    affinity_mat = torch.bmm(query, key.transpose(1, 2)) # (batch_size, seq_len, seq_len)
    affinity_mat = affinity_mat * scale

    if mask is not None:
        affinity_mat = affinity_mat.masked_fill(mask, -inf)
    
    affinity_mat = torch.softmax(affinity_mat, dim=-1) # (batch_size, seq_len, seq_len)

    out = torch.bmm(affinity_mat, value) # (batch_size, seq_len, emb_dim)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
class SingleHeadAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        c = torch.sqrt(torch.tensor(6/(dim_in+dim_v)))
    ...
    def forward(
        ...
        # Transform input queries, keys, and values using Linear layers      #
        # defined above and pass them with mask to                           #
        # scaled_dot_product_no_loop_batch. Store the affinity matrix in     #
        # self.affinity_mat and return the output.                           #
        ######################################################################
        # Replace "pass" with your code (do not modify this line)
        q_proj = self.q(query)
        k_proj = self.k(key)
        ...
        out, affinity_mat = scaled_dot_product_no_loop_batch(q_proj, k_proj, v_proj, mask)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.heads = nn.ModuleList([
            SingleHeadAttention(dim_in, dim_head, dim_head) for _ in range(num_heads)
        ])

        self.proj = nn.Linear(dim_out, dim_in)

        c = torch.sqrt(torch.tensor(6 / (dim_out + dim_in)))
    ...
    def forward(
        ...
        # Replace "pass" with your code (do not modify this line)
        head_outputs = [
            head(query, key, value, mask) for head in self.heads 
        ] 

        concat = torch.cat(head_outputs, dim=-1) # (batch_size, seq_len, dim_out)

        out = self.proj(concat) # (batch_size, seq_len, dim_in)

        for h in range(self.num_heads):
            self.heads[h].affinity_mat = self.heads[h].affinity_mat
        ######################################################################
        #                          END OF YOUR CODE                          #
        ######################################################################
        return out
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        # Do NOT use torch.std, torch.var, and their variants                #
        ######################################################################
        # Replace "pass" with your code (do not modify this line)
        ...
        x_normalized = (x - mean) / std # (batch_size, seq_len, emb_dim)
...
class FFN(nn.Module):
    def __init__(self, inp_dim: int, hid_dim: int):
        ...
        self.act = None
        ######################################################################
        # TODO: Initialize the two-layer feed-forward network by the same    #
        # strategy as described in SingleHeadAttention:                      #
        # For a Linear layer mapping from D_in to D_out dimension, sample    #
        # weights from a uniform distribution bounded by [-c, c], where      #
        # c = sqrt(6/(D_in + D_out))                                         #
        # You need to figure out an appropriate dimension of the output and  #
        # apply it for the second fully-connected layer.                     #
        ######################################################################
        # Replace "pass" with your code (do not modify this line)
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        self.attn = MultiHeadAttention(num_heads=num_heads,
                                       dim_in=emb_dim, dim_out=emb_dim)
...
def get_subsequent_mask(seq):
    ...
    mask = subsequent_mask.unsqueeze(0).expand(batch_size, -1, -1) # (batch_size, seq_len, seq_len)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return mask
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        ######################################################################
        # Replace "pass" with your code (do not modify this line)
        self.attn_mask = MultiHeadAttention(num_heads=num_heads,
                                            dim_in=emb_dim, 
                                            dim_out=emb_dim)
        self.norm1 = LayerNorm(emb_dim=emb_dim)
        self.attn_cross = MultiHeadAttention(num_heads=num_heads, dim_in=emb_dim, 
                                             dim_out=emb_dim)
        self.norm2 = LayerNorm(emb_dim=emb_dim)
        self.ffn = FFN(inp_dim=emb_dim, hid_dim=ffn_dim)
    ...
    def forward(
        ...
        # Replace "pass" with your code (do not modify this line)
        residual = x 
        attn_mask_out = self.attn_mask(x, x, x, mask=mask)
        x = self.dropout(attn_mask_out)
        x = self.norm1(residual + x)

        residual = x
        attn_cross_out = self.attn_cross(x, enc_out, enc_out)
        x = self.dropout(attn_cross_out)
        x = self.norm2(residual + x)

        residual = x 
        ffn_out = self.ffn(x)
        x = self.dropout(ffn_out)
        out = self.norm3(residual + x)
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        ######################################################################
        # Replace "pass" with your code (do not modify this line)
        inp_emb = self.emb_layer(inp_seq.long())
        tgt_emb = self.emb_layer(tgt_seq.long())

        inp_emb = inp_emb + inp_pos
        tgt_emb = tgt_emb + tgt_pos

        enc_out = self.encoder(inp_emb)

        mask = get_subsequent_mask(tgt_seq)

        dec_out = self.decoder(tgt_emb, enc_out, mask)

        dec_out = dec_out[:, :-1, :] 
        out = dec_out.reshape(-1, dec_out.size(-1)) # (batch_size * tgt_len, emb_dim)
        ######################################################################
        #                          END OF YOUR CODE                          #
        ######################################################################
        return out
