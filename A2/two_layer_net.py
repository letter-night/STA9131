...
def nn_forward_backward(
    ...
    # This should include both the data loss and L2 regularization for W1    #
    # and W2. Store the result in the variable `loss`, which is a scalar.    #
    ...
    sum_scores = torch.log(torch.sum(torch.exp(scores), axis=1))
    ...
    ##########################################################################
    # Replace "pass" with your code (do not modify this line)
    dL2 = (torch.exp(scores) / torch.sum(torch.exp(scores), axis=1).reshape((-1, 1))) / N
    dL2[range(scores.shape[0]), y] -= 1 / N 

    dW2 = h1.t() @ dL2 + 2 * W2 * reg
    db2 = torch.sum(dL2, axis=0)
    ...
    dL1 = dL1_relu
    dL1[mask] = 0
    dW1 = X.t() @ dL1 + 2 * W1 * reg
    db1 = torch.sum(dL1, axis=0)
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        if verbose and it % 100 == 0:
            print("iteration %4d / %4d: loss %f" % (it, num_iters, loss.item()))
...
def nn_predict(
    ...
    ##########################################################################
    # Replace "pass" with your code (do not modify this line)
    y_pred = torch.argmax(nn_forward_pass(params, X)[0], axis=1)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def nn_get_search_params():
    ...
    hidden_sizes = [64, 128]
    regularization_strengths = [0.001, 0.0001]
    learning_rate_decays = [0.90, 0.95]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def find_best_net(
    ...
    # qualitative differences from the ones we saw above for the poorly      #
    # tuned network.                                                         #
    #                                                                        #
    ...
    def test_one_param_set(
        ...
        train_dict = cls.train(data_dict['X_train'], data_dict['y_train'],
                               data_dict['X_val'], data_dict['y_val'], learning_rate=lr, learning_rate_decay=lr_decay,
        ...
        return cls, train_dict, train_acc, val_acc
        ...
        for hidden in hidden_sizes:
            for reg in regularization_strengths:
                for lr_decay in learning_rate_decays:
                    i += 1
                    print('Training %d / %d with learning_rate=%e | hidden_size=%d | reg=%e | learning_rate_decay=%e'
                        % (i, num_models, lr, hidden, reg, lr_decay))
                    
                    cand_model, train_dict, cand_train_acc, cand_val_acc = test_one_param_set(
                        TwoLayerNet(3*32*32, hidden, 10, device=data_dict['X_train'].device, 
                                    dtype=data_dict['X_train'].dtype),
                                    data_dict, lr, hidden, reg, lr_decay, num_iters)
                    
                    if cand_val_acc > best_val_acc:
                        best_val_acc = cand_val_acc
                        best_net = cand_model
                    best_stat[(lr, hidden, reg, lr_decay)] = (cand_train_acc, cand_val_acc)
                    print('lr %e hidden %d reg %e lr_decay %e train accuracy: %f val accuracy: %f'
                          % (lr, hidden, reg, lr_decay, cand_train_acc, cand_val_acc))
                    best_stat['loss_history'] = train_dict['loss_history']
                    best_stat['train_acc_history'] = train_dict['train_acc_history']
                    best_stat['val_acc_history'] = train_dict['val_acc_history']
